...
def _sync(unit, amount):
    ...
    for _, v in all_components.items():
        if v["id"] not in components_processed_ids:
            if v["team_id"]:
                row = es.search("tasks_components_coverage", "q=component_id:%s AND topic_id:%s AND team_id:%s" % (v["id"], v["topic_id"], v["team_id"]))
                if len(row) == 0:
                    f_c = format_component_coverage(v, v["team_id"])
                    es.push("tasks_components_coverage", f_c, str(uuid.uuid4()))
            else:
                row = es.search("tasks_components_coverage", "q=component_id:%s AND topic_id:%s AND team_id:%s" % (v["id"], v["topic_id"], "red_hat"))
                if len(row) == 0:
