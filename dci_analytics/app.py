...
_LOCK_FULL_TASK_DURATION_CUMULATED = threading.Lock()
_LOCK_TASK_COMPONENTS_COVERAGE = threading.Lock()
_LOCK_FULL_TASK_COMPONENTS_COVERAGE = threading.Lock()
...
def task_duration_cumulated_sync():
    return lock_and_run(
        _LOCK_TASK_DURATION_CUMULATED, task_duration_cumulated.synchronize
    )


@app.route("/task_duration_cumulated/full_sync", strict_slashes=False, methods=["POST"])
def task_duration_cumulated_full_sync():
    return lock_and_run(
        _LOCK_FULL_TASK_DURATION_CUMULATED, task_duration_cumulated.full_synchronize
    )


@app.route("/task_components_coverage/sync", strict_slashes=False, methods=["POST"])
def task_components_coverage_sync():
    return lock_and_run(
        _LOCK_TASK_COMPONENTS_COVERAGE, task_components_coverage.synchronize
    )
...
def task_components_coverage_full_sync():
    return lock_and_run(
        _LOCK_FULL_TASK_COMPONENTS_COVERAGE, task_components_coverage.full_synchronize
    )
