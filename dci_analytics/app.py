...
@app.route("/pipeline_dashboard/sync", strict_slashes=False, methods=["POST"])
def tasks_telco_sync():
    return lock_and_run(_LOCK_TASK_JUNIT, tasks_pipeline.synchronize)


@app.route("/pipeline_dashboard/full_sync", strict_slashes=False, methods=["POST"])
...
@app.route("/pipeline_dashboard", strict_slashes=False, methods=["POST"])
def pipeline_dashboard():
    ...
    body = {
        "query": {
            "bool": {
                "must": [
                    {"range": {"pipeline.created_at": {"gte": start_date, "lte": end_date}}},
                ]
            }
        },
        "from": 0,
    ...
    if teams_ids:
        ...
        for team_id in teams_ids:
            team_id_should_query["bool"]["should"].append({"term": {"team_id": team_id}})
    ...
    if pipelines_names:
        ...
        for pipeline_name in pipelines_names:
            pipeline_name_should_query["bool"]["should"].append({"term": {"pipeline.name": pipeline_name}})
        body["query"]["bool"]["must"].append(pipeline_name_should_query)

    jobs = []
    while True:
        _jobs = es.search_json("pipeline_dashboard", body)
    ...
    pipelines = {}
    for job in jobs:
        if job["pipeline"]["name"] not in pipelines:
            pipelines[job["pipeline"]["name"]] = {"jobs": [], "created_at": job["pipeline"]["created_at"]}
        pipelines[job["pipeline"]["name"]]["jobs"].append(job)
